...
export class WorkspaceService {
  ...
  createWorkspace(
    data: Partial<Frame> & { backgroundUrl?: File | string | null }
  ): Observable<Frame> {
    const hasFile =
      !!data && !!data.backgroundUrl && typeof data.backgroundUrl !== 'string';

    if (hasFile) {
      const form = new FormData();
      if (data.name !== undefined) form.append('name', String(data.name));
      if (data.visibility !== undefined)
        form.append('visibility', String(Number(data.visibility)));
      if (data.backgroundColor != null)
        form.append('backgroundColor', String(data.backgroundColor));
      form.append('backgroundPath', data.backgroundUrl as File);

      return this.http
        .post<{ message: string; workspace: Frame }>(
  ...
  getWorkspaces(): Observable<Frame[]> {
    return this.http
      .get<{ message: string; workspaces: Frame[] }>(
  ...
  getWorkspaceById(id: number): Observable<Frame> {
    return this.http
      .get<{ message: string; workspace: Frame }>(
  ...
  updateWorkspace(
    ...
    if (hasFile) {
      ...
      return this.http
        .put<{ message: string; workspace: Frame }>(
          `${this.apiUrl}/workspace/${id}`,
          form
        )
        .pipe(map((res) => res.workspace));
  ...
  removeCollaborator(workspaceId: number, userId: number): Observable<void> {
    return this.http.request<void>(
      'DELETE',
      `${this.apiUrl}/workspace/collaborators`,
      {
